            ...
            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                <div class="product-status-wrap">
                    ...
                    <div class="form-group">
                        ...
                        <form id="editBlogWidgetForm" action="{{route('commentWidget.update', $widget->id)}}" method="POST" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="form-group custom-select-container">
                                ...
                                <select id="comments" name="comments[]" class="custom-select" multiple>
                                    @foreach ($comments as $comment)
                                        <option value="{{$comment->id}}" @if(in_array($comment->id, $widget->comments->pluck('id')->toArray())) selected @endif>{{$comment->content}}</option>
                                    @endforeach
                                </select>
                            </div>
